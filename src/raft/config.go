...
func (cfg *config) checkOneLeader() int {
	for iters := 0; iters < 10; iters++ {
		log.Printf("checkoneleader turn %v", iters)
		ms := 450 + (rand.Int63() % 100)
		time.Sleep(time.Duration(ms) * time.Millisecond)

		leaders := make(map[int][]int)
		for i := 0; i < cfg.n; i++ {
			log.Printf("%v if out", cfg.rafts[i].me)
			if cfg.connected[i] {
				log.Printf("%v--++", cfg.rafts[i].me)
				if term, leader := cfg.rafts[i].GetState(); leader {
					leaders[term] = append(leaders[term], i)
				}
			}
		}
		log.Printf("checkoneleader turn2 %v", iters)
		...
		log.Printf("checkoneleader turn1 %v", iters)

		if len(leaders) != 0 {
			return leaders[lastTermWithLeader][0]
		}
	}
	log.Println("expected one leader, got none")
...
func (cfg *config) checkTerms() int {
	...
	for i := 0; i < cfg.n; i++ {
		if cfg.connected[i] {
			...
			fmt.Println(strconv.Itoa(cfg.rafts[i].me) + "----" + strconv.Itoa(xterm))
			if term == -1 {
				term = xterm
			} else if term != xterm {
				log.Println("servers disagree on term")
...
func (cfg *config) checkNoLeader() {
	for i := 0; i < cfg.n; i++ {
		if cfg.connected[i] {
			...
			if is_leader {
				log.Println("expected no leader, but " + strconv.Itoa(i) + " claims to be leader")
...
func (cfg *config) nCommitted(index int) (int, interface{}) {
	...
	for i := 0; i < len(cfg.rafts); i++ {
		...
		if ok {
			log.Printf("count : %v , cmd : %+v , cmd1 : %+v", count, cmd, cmd1)
			if count > 0 && cmd != cmd1 {
...
func (cfg *config) one(cmd interface{}, expectedServers int, retry bool) int {
	...
	for time.Since(t0).Seconds() < 10 {
		...
		for si := 0; si < cfg.n; si++ {
			log.Printf("start test si %v , command : %+v", si, cmd)
			...
			if rf != nil {
				...
				log.Printf("test--- index1 : %v, ok : %v, S%v", index1, ok, rf.me)
				if ok {
					index = index1
					break
				}
		...
		if index != -1 {
			...
			for time.Since(t1).Seconds() < 2 {
				log.Printf("n11 : %v", n11)
				nd, cmd1 := cfg.nCommitted(index)
				log.Printf("n11 : %v, nd : %v", n11, nd)
